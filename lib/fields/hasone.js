	...
	,hydrate: function( bundle, cb ){
		this.parent('hydrate', bundle,( err, value )=>{
			return this.toInstance( value,(err, instance)=>{
				this.instance.full_hydrate( { req:bundle.req, res:bundle.res, data:instance},(err, result, saved)=>{
	...
	,toInstance: function( value, cb ){
		...
		switch(kindOf(value)){
			case 'String':
				...
				bits = value.split('/');
				value = {};
				value[this.instance.options.objectTpl._pk] = bits[bits.length-1];
				saved = true;
				break;
			case 'Object':
